...
export const transformImageAction = actionClient
  ...
  .action(async ({ parsedInput: { imageBase64, sessionId: inputSessionId } }) => {
    ...
    const contentType = imageBase64.match(/^data:(image\/\w+);base64,/)?.[1] || "image/png";

    // Get upload URL from Convex
    const uploadUrl = await convex.mutation(api.submissions.generateUploadUrl);
    ...
    after(async () => {
      try {
        ...
        const sourceImageUrl = await convex.query(api.submissions.getSourceImageUrl, {
          id: submissionId,
        });
        ...
        // Get the result URL
        const resultUrl = typeof output === "object" && "url" in output
        ...
        const resultUploadUrl = await convex.mutation(api.submissions.generateUploadUrl);
        ...
        const { storageId: resultStorageId } = await resultUploadResponse.json();
